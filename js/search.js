document.addEventListener('DOMContentLoaded', function () {
    ...
    async function searchSite(query) {
        // Recherche toutes les pages dans le dossier /pages/ dynamiquement
        let pagesToSearch = [
            '/index.html',
            '/contact.html'
        ];

        // Ajoute dynamiquement toutes les pages du dossier /pages/
        try {
            ...
            if (href && href.endsWith('.html') && !pagesToSearch.includes('pages/' + href)) {
                pagesToSearch.push('pages/' + href);
            }
            });
        } catch (e) {
            // Si l'indexation dynamique échoue, fallback sur la liste statique
            pagesToSearch = pagesToSearch.concat([
            'pages/adjib.html',
